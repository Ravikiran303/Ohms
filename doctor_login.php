...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
  $name=$_POST['doc_id'];
  $pass=$_POST['password'];
  $sql = "SELECT * FROM doctor_details where doc_id = '$name'";
  ...
  if($row["doc_id"]==$name && $row["password"]==$pass)
  {
    $_SESSION["id"] = $row["id"];
    header("Location: doctor_home.php");
  }
...
<body>
  ...
  <form class="p-5" method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>">
    <img src="">
    <p class="h4 mb-4 text-center" style="border-radius: 5px;height:30px; font-size: 20px;font-family: Bookman Old style; font-weight: bold;">Doctor Sign In</p>

    <input type="text" id="defaultLoginFormEmail" class="form-control mb-4" placeholder="University ID" name="doc_id">
